...
function strToInt1(str: string) {
    let c: string = str.trim();
    if (c.length == 0) return 0;
    let res: number = 0, bndry: number = Number.MAX_VALUE / 10;
    let i: number = 1, sign: number = 1;
    if (c[0] == '-') sign = -1;
    else if (c[0] != '+') i = 0;
    for (let j: number = i; j < c.length; j++) {
...
function strToInt2(str: string) {
    let res: number = 0, bndry: number = Number.MAX_VALUE / 10;
    let i: number = 0, sign: number = 1, length: number = str.length
